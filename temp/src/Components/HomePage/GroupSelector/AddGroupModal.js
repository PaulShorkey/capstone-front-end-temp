...
export default function AddGroupModal(props) {
	...
	const handleAddGroup = () => {
		const name = document.getElementById('add-group-name-input').value
		const description = document.getElementById('add-group-description-input').value
		let errorMessage = ''

		if (name === undefined || typeof name !== 'string' || name === '') {
			errorMessage += 'Please Provide a Value for Group Name\n'
		}
		if (description === undefined || typeof description !== 'string' || description === '') {
			errorMessage += 'Please Provide a Value for Group Description'
		}
				...
				<Fade in={open}>
					<div className={classes.paper}>
						...
						<form className={classes.root} noValidate autoComplete='off'>
							...
							/>
							<TextField
								id='add-group-description-input'
